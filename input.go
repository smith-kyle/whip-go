	...
	for {
		...
		if len(nalData) >= 5 {
			...
			if nalType == 9 {
				...
				continue // Skip this NAL unit, process next one from buffer
			}
		}

		// This is not an AUD, process it
		log.Printf("Processing NAL unit type %d, size: %d bytes", nalData[4]&0x1F, len(nalData))
